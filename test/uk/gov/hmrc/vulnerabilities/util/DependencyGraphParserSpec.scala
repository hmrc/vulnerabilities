...
class DependencyGraphParserSpec
  ...
  "DependencyGraphParser.pathsToRoot" should:
    "return path to root" in:
      val source = scala.io.Source.fromResource("graphs/dependencies-compile.dot")
      val graph  = DependencyGraphParser.parse(source.mkString)
      graph.pathsToRoot(Node("org.typelevel:cats-kernel_2.12:2.2.0")) shouldBe List(List(
        Node("org.typelevel:cats-kernel_2.12:2.2.0"),
        Node("org.typelevel:cats-core_2.12:2.2.0"),
        Node("uk.gov.hmrc:my-slug_2.12:2.22.0")
      ))
    ...
    "work with maven dependencies" in:
      val source = scala.io.Source.fromResource("graphs/dependencies-maven.dot")
      val graph  = DependencyGraphParser.parse(source.mkString)
      graph.pathsToRoot(Node("javax.xml.stream:stax-api:jar:1.0-2:compile")) shouldBe List(List(
        Node("javax.xml.stream:stax-api:jar:1.0-2:compile"),
        Node("org.springframework.ws:spring-xml:jar:2.1.4.RELEASE:compile"),
        Node("org.springframework.ws:spring-ws-support:jar:2.1.4.RELEASE:compile"),
        Node("uk.gov.hmrc.jdc:emcs:war:3.226.0")
    ...
    "work with emcs dependencies" in:
      ...
      val graph  = DependencyGraphParser.parse(source.mkString)
      graph.nodes.map(d => (d.group, d.artefact, d.version, d.scalaVersion)) should not be empty
    ...
    // BDOG-1884 if this test is hanging, its because pathsToRoot's cycle detection has broken
    "not get stuck in an infinite loop when parsing a cyclical graph" in:
      ...
      graph.pathsToRoot(baz) shouldBe List(List(Node("org:baz:3.0.0"), Node("org:bar:2.0.0")))
      val bar    = graph.nodes.filter(_.artefact == "bar").head
      graph.pathsToRoot(bar) shouldBe List(List(Node("org:bar:2.0.0"), Node("org:baz:3.0.0")))

    "return multiple paths" in:
      ...
      graph.pathsToRoot(sbtSettings) shouldBe List(
        List(Node("uk.gov.hmrc:sbt-settings:0.0.1"), Node("uk.gov.hmrc:sbt-auto-build:3.0.0"), Node("default:project:0.1.0-SNAPSHOT")),
        List(Node("uk.gov.hmrc:sbt-settings:0.0.1"), Node("default:project:0.1.0-SNAPSHOT"))
      )
    ...
    "ignore evicted nodes" in:
      ...
      graph.pathsToRoot(log4j) shouldBe List(List(
