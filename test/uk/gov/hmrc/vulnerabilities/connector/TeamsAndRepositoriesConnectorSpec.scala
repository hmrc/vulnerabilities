...
class TeamsAndRepositoriesConnectorSpec
  ...
  private val servicesConfig = ServicesConfig(
    Configuration(
      ...
      "microservice.services.teams-and-repositories.cache.expiration" -> "0.seconds"
    )
  )
  private val cache = mock[AsyncCacheApi]

  private val connector = TeamsAndRepositoriesConnector(servicesConfig, httpClientV2, cache)
  ...
  "TeamsAndRepositoriesConnector.repositories" should:
    "return all the repositories" in:
      ...
      connector.repositories(teamName = None).futureValue shouldBe Seq(
        Repo("service1", Seq("team1", "team2")),
        Repo("service2", Seq("team1", "team3"))
      )
