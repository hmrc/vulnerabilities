...
class XrayService @Inject()(
  ...
  def rescanStaleReports(reportsBefore: Instant)(implicit hc: HeaderCarrier): Future[Unit] =
    for {
      staleReports <- rawReportsRepository.findStale(reportsBefore)
      _            <- deleteStaleReports()
      slugs        =  staleReports.map(report => SlugInfo(
  ...
  private def generateReport(slug: SlugInfo)(implicit hc: HeaderCarrier): EitherT[Future, Status, Report] =
    for {
      ...
      _       =  logger.info(s"Began generating report for ${slug.serviceName.asString}:${slug.version.original}. Report will have id ${resp.reportID}")
      ...
      report  <- status match {
                   ...
                   case XrayNoData   => for {
                                          ...
                                          _   =  logger.info(s"${slug.serviceName.asString}:${slug.version.original} - ${status.statusMessage} Report ${resp.reportID} has been deleted from the Xray UI")
