...
@Singleton
class DeploymentQueueConfig @Inject()(
  configuration: Configuration
) {
  private lazy val sqsQueueUrlPrefix =
    new URI(configuration.get[String]("update.vulnerabilities.aws.sqs.queue-prefix")).toURL

  private lazy val sqsQueueDeploymentInfo =
    configuration.get[String]("update.vulnerabilities.aws.sqs.queue-deployment")
  ...
  lazy val isEnabled             : Boolean = configuration.get[Boolean]("update.vulnerabilities.enabled")
}
