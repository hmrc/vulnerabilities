...
@Singleton
class RemoveOldReportsScheduler @Inject()(
  ...
  private val schedulerConfigs =
    SchedulerConfig(configuration, "scheduler.remove-old-reports")

  private val lock: ScheduledLockService =
    ScheduledLockService(
      lockRepository    = mongoLockRepository,
      lockId            = "remove-old-reports-lock",
      timestampSupport  = timestampSupport,
  ...
  scheduleWithLock("RemoveOldReportsScheduler", schedulerConfigs, lock):
    xrayService.removeOldReports()
