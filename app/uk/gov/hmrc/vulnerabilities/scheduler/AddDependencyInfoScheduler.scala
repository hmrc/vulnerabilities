...
@Singleton
class AddDependencyInfoScheduler @Inject()(
  ...
  private val schedulerConfigs =
    SchedulerConfig(configuration, "scheduler.add-dependency-info")

  private val lock: ScheduledLockService =
    ScheduledLockService(
      lockRepository    = mongoLockRepository,
      lockId            = "add-dependency-info-lock",
      timestampSupport  = timestampSupport,
  ...
  scheduleWithLock("Add Dependency Info", schedulerConfigs, lock):
    xrayService.addDependencyInfoScheduler()
